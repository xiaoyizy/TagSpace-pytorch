...
class TagSpace(nn.Module):
	...
	# 1. get embed
		post_embed = self.embed(input).to(self.device)
#		 print('Size after emebedding:', post_embed.size())
	# 2. convolution + tanh activation
		post_conv = torch.tanh(self.conv(post_embed.permute(0, 2, 1))).to(self.device)
#		 print('Size after convolution layer:', post_conv.size())
	# 3. maxpool + tanh activation
		post_maxpool = torch.tanh(self.maxpool(post_conv).reshape(self.batch_size, self.hidden_size)).to(self.device)
#		 print('Size after max pooling:', post_maxpool.size())
	# 4. linear decoder
		tweets_embed = self.decoder(post_maxpool).to(self.device)
#		 print('Size of output:', tweets_embed.size())
			
		return tweets_embed
		
	
	def WARPLoss(self, tweets_emb, labels, hashtag_pool):
		"""
		Compute the averaged WARP loss of a batch of tweets
		-Args:
			@tweets_emb: tensor, 
			@labels: list, list of index lists of hashtags found in the tweet batch
		...
		pos_embs = self.embed(labels).to(self.device)
		pos_scr = torch.bmm(torch.unsqueeze(tweets_emb, 1), torch.unsqueeze(pos_embs, 2)).squeeze().to(self.device)
		# print('Positive pairs generated.')
		
		# generate negative pairs:
		neg_embs = torch.zeros(self.batch_size, self.embedding_size).to(self.device)
		for b in range(self.batch_size): # loop over tweets in the batch
			neg_samples = torch.LongTensor(hashtag_pool)[
				torch.multinomial(torch.ones(len(hashtag_pool)), self.max_iter)].to(self.device)
			for i, neg_sample in enumerate(neg_samples): # neg_sample is an tensor of one index
				neg_emb = self.embed(neg_sample).to(self.device)
				if torch.dot(tweets_emb[b], neg_emb) >= pos_scr[b] or i == self.max_iter-1:
					neg_embs[b] = neg_emb.to(self.device)
		# print('Negative pairs generated.')
					
		# calculate WARP loss
		neg_scr = torch.bmm(torch.unsqueeze(tweets_emb, 1), 
						 torch.unsqueeze(neg_embs, 2)).squeeze().to(self.device)
		margin_stacked = torch.Tensor(torch.ones(self.batch_size)*self.margin).to(self.device)
		warp_loss = torch.mean(torch.max(torch.zeros(self.batch_size).to(self.device), margin_stacked-pos_scr + neg_scr)).to(self.device)
		# print('Calculated WARP loss:', warp_loss)
		return warp_loss

	def embed_hashtag_pool(self, hashtag_pool):
		hashtag_pool_tensor = torch.Tensor(len(hashtag_pool), self.embedding_size).to(self.device)
		for i in range(len(hashtag_pool)):
			hashtag_pool_tensor[i] = self.embed(torch.LongTensor([hashtag_pool[i]]).to(self.device))
